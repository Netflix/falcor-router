...
function callOnErrorWithException(routerInstance, matchAndPath, note) {
    ...
    if (kind !== onNext) {
        ...
        // Make it possible for the callback to transform 
        // an Error to another Error or JSONGraphError
        var transformedError = 
            routerInstance.onError(method, pathOrPathSet, note.error);
        
        if (transformedError) {
            if (transformedError instanceof Error 
                || transformedError instanceof JSONGraphError) {
                note.error = transformedError;
            }
            else {
                throw new Error("onError callback should tranform Error to " + 
                    "Error or JSONGraphError");
            }
        }
    }
